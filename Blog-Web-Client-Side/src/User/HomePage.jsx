...
export default function HomePage() {
  ...
  const { View, loading, setLoading, token, validToken, userData } =
    useContext(AppContextContainer);
  ...
  useEffect(() => {
    ...
    if (!loading) {
      ...
      axios
        .get(`${API_LINK}/story/posts`, { headers: { Authorization: token } })
        .then((res) => {
  ...
  function fetchPosts(quantity) {
    if (!submittedSearch) {
      axios
        .get(`${API_LINK}/story/posts/${quantity}`, {
      ...
      axios
        .get(`${API_LINK}/story/postsSearch/${submittedSearch}&${quantity}`, {
          headers: { Authorization: token },
        })
        .then((res) => {
          setFetchingPosts(false);
  ...
  function deletePost(id) {
    axios
      .delete(`${API_LINK}/story/delete/${id}`, {
        headers: {
          Authorization: token,
        },
      })
      .then((res) => {
        ...
        alert(
          "You Deleted it successfully,Changes will apear after refreshing of page.",
        );
      })
      .catch((err) => console.log(err));
  }
  function searchHandler(e) {
    ...
    if (search) {
      axios
        .get(`${API_LINK}/story/postsSearch/${search}&0`, {
          headers: { Authorization: token },
        })
        .then((res) => {
          if (res.data?.msg) {
      ...
      >
        {stories.length >= 1 &&
          stories.map((each, index) => {
            return (
              <div key={each._id} className="posts">
                <div className="postHeader">
                  <div className="postAuthor">
                    <Link to={`/user/${each.author._id}`}>
                      <img
                        src={
                          each.author.isOauthUser
                            ? `${each.author.path}`
                            : `${HOST_LINK}/${each.author.path}`
                        }
